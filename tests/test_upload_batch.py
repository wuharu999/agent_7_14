...
class UploadBatchTemplateTests(unittest.TestCase):
    ...
    def test_global_upload_status_is_persisted_and_refreshed_from_the_api(self) -> None:
        ...
        current_panel_start = self.template.index('<details id="current-batch-panel"')
        current_panel_end = self.template.index("</details>", current_panel_start)
        upload_queue = self.template.index('id="upload-queue"')
        self.assertLess(current_panel_start, upload_queue)
        self.assertLess(upload_queue, current_panel_end)
        ...
        self.assertIn("link.href=window.appUrl('/uploads/'+encodeURIComponent", self.template)
        self.assertIn("xhr.open('POST',window.appUrl('/upload'))", self.template)
