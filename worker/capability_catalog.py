...
def inspect_capability_source_changes(model_id: str) -> dict[str, Any]:
    ...
    previous = _load_source_manifest(target)
    ...
    *,
    ...
    try:
        ...
        return {
            ...
            "coverage_report": changeset.get("coverage_report") or {},
            "source_changes": source_changes,
            "current_source_files": len(source_manifest),
            "last_organized_manifest_files": len(source_manifest),
        }
    except Exception:
...
async def organize_capability_catalog(
    *,
    ...
    source_files = [path for path in team_config.raw_sources_dir.rglob("*") if path.is_file() and not path.is_symlink()]
    if not source_files:
    ...
    previous_manifest = await asyncio.to_thread(_load_source_manifest, target)
    ...
    change_count = int(source_changes["counts"]["total"])
    await on_progress(
        "inventorying",
        f"Claude is inventorying model sources. {change_count} source changes were detected since the last organization.",
    )
    system_prompt = (
        "You are an atomic capability Wiki maintainer running in a controlled draft pipeline. "
        "Follow the embedded maintenance skill and references exactly. Read the requested model's "
        "raw source corpus and relevant Wiki pages silently with Read, Glob, and Grep. Treat every "
    ...
    prompt = (
        ...
        f"Source corpus: raw/sources/{model}/\n"
        f"Supporting Wiki evidence: wiki/\n"
        f"Target Wiki section: wiki/capabilities/{model}/\n"
        f"Target base revision: {base_revision}\n\n"
        "Inventory every in-scope source. Extract independently triggerable and reusable atomic "
